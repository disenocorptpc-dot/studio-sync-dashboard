...
import { getFirestore, collection, addDoc, onSnapshot, doc, updateDoc, deleteDoc, query, orderBy, writeBatch, getDocs, getDoc } from "firebase/firestore";
...
let scenes = [];
let sharedGeometry = new THREE.BoxGeometry(1, 1, 1); // Fallback geometry
let projects = [];
...
function init() {
    ...
    style.textContent = `
        ...
        .notes-area { width: 100%; height: 80px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 4px; color: #ccc; padding: 8px; font-family: 'Outfit'; resize: vertical; outline: none; margin-top: 5px; }
        .notes-area:focus { background: rgba(0,0,0,0.5); border-color: #00f3ff; color: #fff; }
...
function subscribeToProjects() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        projects.forEach(p => {
            // Debug: Auto-trigger download if missing
...
async function downloadModelChunks(id) {
    ...
    try {
        ...
        if (fullText.length > 50) {
            ...
            if (dbg) dbg.innerText = "Empty Data.";
        }
    } catch (e) {
        console.error("Download Error", e);
        if (dbg) dbg.innerText = "Error: " + e.message;
    } finally {
        ...
        card.innerHTML = `
            ...
            <div class="controls-section">
                ...
                <div style="margin-top:15px;padding-top:10px;border-top:1px solid rgba(255,255,255,0.1);display:flex;justify-content:space-between;align-items:center;">
                    ...
                    <button onclick="document.getElementById('n-${proj.id}').style.display=document.getElementById('n-${proj.id}').style.display==='none'?'block':'none'" style="background:#00f3ff22;color:#00f3ff;border:1px solid #00f3ff;padding:5px;font-size:0.75rem;">📋 BITÁCORA</button>
                </div>
                <div id="n-${proj.id}" style="display:none;margin-top:5px;"><textarea class="notes-area" onblur="window.updateNotes('${proj.id}', this.value)">${proj.notes}</textarea></div>
            </div>
...
// UTILS
window.addProject = async () => { try { await addDoc(collection(db, "projects"), { name: "Nuevo Proyecto", client: "Cliente...", deadline: new Date().toISOString().split('T')[0], progress: 0, order: projects.length, responsible: Array(6).fill(""), notes: "", phaseStarts: Array(6).fill(""), phaseEnds: Array(6).fill("") }); } catch (e) { console.error(e); } };
...
window.updateNotes = (id, v) => debounceUpdate(id, { notes: v });
